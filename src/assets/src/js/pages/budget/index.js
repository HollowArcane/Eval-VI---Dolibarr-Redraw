...
class Page extends BasePage
{
    ...
    {
        request(`/api/budget?page=${page}&date_min=${formdata.get('date-min')}&date_max=${formdata.get('date-max')}`)
    ...
    update(row)
    {
        request(`/api/budget`).authorize(this.token).json(row).put()
        .then(json => {
    ...
    delete(row)
    {
        ...
        .then(response => {
            ...
            {
                request(`/api/budget/${row.id}`).authorize(this.token).delete()
                    .then(json => {
                        alerts.success('', json.data.message);
                        this.read(this.form.formData());
                    })
    ...
    render(data)
    {
        this.table.replaceChildren(...data.data.items.map(row => {
            ...
            return tag('tr', {'class': 'table-row', 'style': 'opacity: 0'}, [
                ...
                tag('td', {}, [text(this.formatDate(row.budgetDate))]),
